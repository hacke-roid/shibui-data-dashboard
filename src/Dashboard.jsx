...
function Dashboard() {
  ...
  useEffect(() => {
    const loadUserConfig = async () => {
      ...
      try {
        console.log(
          `Fetching user configuration from: http://localhost:3002/load-configuration/${user.id}`
        );
        const response = await fetch(
          `http://localhost:3002/load-configuration/${user.id}`
        );
  ...
  const authorization = async () => {
    try {
      ...
      let auth = await axios.get("http://localhost:5000/api/v1/user/auth", {
        headers: {
